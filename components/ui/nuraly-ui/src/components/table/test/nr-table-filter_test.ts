import '../components/nr-table-filter';
import {HyTableFilter} from '../components/nr-table-filter';
import {fixture, html, expect} from '@open-wc/testing';
suite('HyTableFilterComponent', () => {
  test('init input filter', async () => {
    const el: HyTableFilter = await fixture(html`<nr-table-filter></nr-table-filter>`);
  ...
  test('dispatch value change', async () => {
    const el: HyTableFilter = await fixture(html`<nr-table-filter .showInput=${true}></nr-table-filter>`);
    const input: HTMLInputElement = el.shadowRoot!.querySelector('input')!;
